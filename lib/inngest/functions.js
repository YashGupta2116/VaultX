...
export const triggerRecurringTransaction = inngest.createFunction(
  ...
  {cron: '0 0 * * *'},
  async ({step}) => {
    // 1. fetch alll due recurring transactions
  ...
  async ({event, step}) => {
    ...
    await step.run('process-transaction', async () => {
      ...
      await db.$transaction(async (tx) => {
        ...
        // udpate last processed date and next recurring date
...
async function getMonthlyStats(userId, month) {
  ...
  return transactions.reduce(
    (stats, t) => {
      const amount = parseFloat(t.amount.toNumber().toFixed(2));
      if (t.type === 'EXPENSE') {
